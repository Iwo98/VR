...
public class ClickWN : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (GetComponent<GameCycleWhatsNew>().areNewItemsShown)
            itemPicking();
    ...
    void itemPicking() {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            foreach (ClickOnWN clickItem in clickOns)
                ...
                if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out rayHit, 100, clickablesLayer))
                {
                    if (clickItem != null)
                    {
                        if (rayHit.collider.GetComponent<ClickOnWN>() != clickItem.GetComponent<ClickOnWN>())
                        {
                            //reset every other to original material and selection state
                            clickItem.GetComponent<ClickOnWN>().resetMaterial();
                            clickItem.GetComponent<ClickOnWN>().setNotSelected();
                        }
                        else
                            //do proper action to clicked object
                            rayHit.collider.GetComponent<ClickOnWN>().ClickMe();
                    }
        ...
        if (Input.GetKeyDown("r") && !Input.GetKey(KeyCode.LeftControl))
            foreach (ClickOnWN clickItem in clickOns)
                if (clickItem != null)
                    if (clickItem.GetComponent<ClickOnWN>().getSelected())
                        clickItem.GetComponent<ClickOnWN>().destroyItem();
    }
    ...
    public void ChoseItemsForGame(int gameLevel) {
        List<ClickOnWN> tempClickOns = new List<ClickOnWN>();
        tempClickOns.AddRange((ClickOnWN[])GameObject.FindObjectsOfType(typeof(ClickOnWN)));

        for (int i = 0; i < (gameLevel + 1) * 3; i++)
        {
            int randomID = rnd.Next(tempClickOns.Count);
            ClickOnWN tempClickOn = tempClickOns[randomID];
