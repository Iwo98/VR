...
public class HandPresenceMichal : MonoBehaviour
{
    ...
    public float clickDeadZone;
    private float clickEnterValue;
    ...
    void UpdateAnimation()
    {
        if (targetDevice.TryGetFeatureValue(CommonUsages.trigger, out float triggerValue) && triggerValue > 0)
        {
            ...
            if (triggerValue > clickThreshold && isClicked == false)
            {
                isClicked = true;
                clickEnterValue = triggerValue;
                onClick.Invoke();
            }
            else if (triggerValue < clickEnterValue - clickDeadZone && isClicked == true)
            {
                isClicked = false;
            }
        }
